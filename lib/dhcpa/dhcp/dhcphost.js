...
var DHCPA;
var Q = require('/Projects/Web/js-attributes');
...
function DHCPHost(opts) {
	...
	var config = {
		address: (opts && opts.address) || '127.0.0.1',
		port: (opts && +opts.port) || 0,
		hostname: (opts && opts.hostname) || 'dhcp'
	};

	var chainable = (name, options) => {
		return Q(this, name, options)
	};
...
DHCPHost.prototype.bind = function(host, port, callback) {
    if (!port) port = 67;
...
function broadcastMessage(msg, address, callback) {
