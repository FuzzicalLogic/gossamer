...
function DHCPAClient(options) {
    ...
    this.client.on('message', (msg, rinfo) => {
        ...
        case DHCPA.protocol.DHCPMessageType.DHCP_OFFER:
            this.emit('offer', rinfo, pkt);
            break;
        case DHCPA.protocol.DHCPMessageType.DHCP_ACK:
            this.emit('acknowledge', pkt);
            break;
        case DHCPA.protocol.DHCPMessageType.DHCP_NAK:
            this.emit('deny', pkt);
            break;
        default:
...
util.inherits(DHCPAClient, EventEmitter);

DHCPAClient.prototype.bind = function(host, port, cb) {
    var that = this;
    if (!port) port = 68;
...
DHCPAClient.prototype.broadcastPacket = function(pkt, options, cb) {
    var port = 67;
...
DHCPAClient.prototype.createDiscoverPacket = function(user) {
	...
	console.log('Creating DHCP/A Discover packet:', util.inspect(pkt, false, 3));
    var actual = DHCPAClient.prototype.createPacket(pkt);
	console.log('Checking DHCP/A Discover buffer:');
	hex(actual);

	//console.log('Checking DHCP/A Discover packet:', util.inspect(parser.parse(actual), false, 3));
	return actual;
}

DHCPAClient.prototype.createRequestPacket = function(user) {
	user.options.dhcpMessageType = DHCPA.protocol.DHCPMessageType.DHCP_REQUEST.value;
