...
function DHCPAServer(options) {
    ...
    this.server.on('message', (msg, rinfo) => {
		...
		case DHCPA.Message.TYPES.DHCP_DISCOVER:
			this.emit('discover', rinfo, pkt);
			break;
		case DHCPA.Message.TYPES.DHCP_REQUEST:
			this.emit('request', rinfo, pkt);
			break;
		case DHCPA.Message.TYPES.DHCP_RELEASE:
...
util.inherits(DHCPAServer, EventEmitter);

DHCPAServer.prototype.bind = function(host, port) {
    if (!port) port = 67;
...
DHCPAServer.prototype.send = function(pkt, port, host, cb) {
	console.log('Sending Packet to:' + host + ':' + port);
	this.server.send(pkt, 0, pkt.length, port, host, cb);
}

DHCPAServer.prototype.createPacket = function(pkt) {
    if (!('xid' in pkt))
...
DHCPAServer.prototype.createOfferPacket = function(user) {
	user.options.dhcpMessageType = DHCPA.Message.TYPES.DHCP_OFFER.value;
